...
## GC-WM Custom Additions (this repo)

### 1) 파일 구조
- `gaussian_initiailization/`: 메인 알고리즘(학습/렌더링/데이터 로딩 등)
  - `train.py`: 학습 루프, 렌더링, densify, 체크포인트
  - `render.py`: 학습 완료 모델로 train/test 뷰 렌더링
  - `scene/`: 카메라/가우시안 모델/데이터셋 처리
  - `gaussian_renderer/`: GPU rasterizer 래퍼, render() 함수
  - `arguments/`: CLI 파라미터 정의 및 병합
  - `output/`: 체크포인트와 렌더 결과 저장
- `SIBR_viewers/`, `third_party/`: 외부 뷰어/라이브러리 종속성

### 2) 설치 방법 (간단)
1. `git clone --recursive https://github.com/graphdeco-inria/gaussian-splatting` (또는 이 repo URL)
2. `cd gaussian_initiailization`
3. `conda env create --file environment.yml`
4. `conda activate gaussian_splatting` (또는 `cg-wm`)
...
- 렌더링 (저해상도 강제, OOM 회피):
  ...
  python render.py \
    ...
    --skip_train
  ```
- alternation (geometry/appearance):
  ```bash
  python train.py --source_path ... --model_path ... --iterations 30000 --eval --disable_viewer --quiet \
...
- checkpoint 저장/재개:
  ```bash
  python train.py --source_path ... --model_path output/lego_resume --iterations 30000 \
    --checkpoint_iterations 5000 10000 20000 --eval --disable_viewer --quiet

  python train.py --source_path ... --model_path output/lego_resume --iterations 30000 \
    --start_checkpoint output/lego_resume/chkpnt10000.pth --eval --disable_viewer --quiet
  ```

### 4) 변경/추가 기능 (GC-WM)
- 3D Gaussian color 확산 문제 분석 및 PSNR 검증 도구 추가.
- `train.py`: geometry/appearance 번갈아 학습 옵션 `--alternating_optimization` 추가.
- `train.py`: `--joint_optimization` 및 체크포인트 재개 시 optimizer/exposure 상태 복구.
- `scene/gaussian_model.py`: rotation 고정 상태(등방성 scaling), `update_learning_rate` 및 optimizer 분리.
- alternating/joint 모드에서도 `feature_lr`, `opacity_lr`, `scaling_lr`가 각각 따로 적용되도록 정리.
- `render.py` 및 `gaussian_renderer`: `RGB2SH`/clamp 이슈 확인 가능.

### 5) 현 상태 요약
- 10k/30k/50k 학습 실행 가능, 200개 테스트 뷰 렌더 가능
- PSNR이 아직 낮은 편이며, alternating/joint는 기능 정합성은 맞췄고 품질 튜닝은 계속 필요
- 현재 GPU(11.56GB)에서 full-res OOM 회피: `--resolution 4` 필수

### 5-1) 실행 검증 메모
- `render.py`:
  - `output/lego_test_joint` 기준 실행 확인
  - 기본 해상도는 현재 GPU(11.56GB)에서 OOM 가능
  - `--resolution 4` 사용 시 test view 200장 렌더 완료
- `train.py`:
...
### 6) 향후 개선 제안
- SH coefficient clipping/regularization (loss term)
- exposure/opacity 스케일 안정화
- 3D gaussian 방향성(비등방성) 강화 옵션 추가
- full 1080p 렌더 optimization (tile/streaming, 메모리 절약)

---

## 바로가기
- `gaussian_initiailization/README.md` 본문은 위의 기존 설명과 반영되어 있습니다.
- 추가 패치 및 실험 결과는 `output/` 하위에 자동 저장됩니다.
