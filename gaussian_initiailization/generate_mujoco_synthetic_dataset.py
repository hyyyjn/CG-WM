...
def parse_args():
    ...
    parser.add_argument("--scene_name", default="simple_box", type=str)
    parser.add_argument("--object_type", default="box", choices=["box", "sphere", "cylinder"])
    parser.add_argument("--train_views", default=24, type=int)
    parser.add_argument("--test_views", default=8, type=int)
    parser.add_argument("--width", default=512, type=int)
    parser.add_argument("--height", default=512, type=int)
    parser.add_argument("--fovy_deg", default=45.0, type=float)
    parser.add_argument("--camera_radius", default=1.45, type=float)
    parser.add_argument("--elevation_deg", default=25.0, type=float)
    parser.add_argument("--object_height", default=None, type=float)
    parser.add_argument("--settle_steps", default=200, type=int)
    parser.add_argument("--mujoco_gl", default="auto", choices=["auto", "egl", "osmesa", "glfw"])
    parser.add_argument("--seed", default=0, type=int)
    parser.add_argument("--add_pose_jitter", action="store_true")
    return parser.parse_args()


def resolve_mujoco_gl(mode):
    if mode == "auto":
        return os.environ.get("MUJOCO_GL", "egl")
    return mode


def object_geom_xml(object_type):
    if object_type == "box":
        return (
            """
...
def build_model_xml(object_type, fovy_deg, width, height):
...
def camera_to_world_matrix(eye, target, up_hint):
...
def orbit_camera_positions(count, radius, elevation_deg, lookat):
    positions = []
...
def set_camera_pose(mujoco, model, camera_name, eye, target, up_hint):
...
def save_png(imageio, path, array):
    path.parent.mkdir(parents=True, exist_ok=True)
    imageio.imwrite(path, array)


def segmentation_to_mask(segmentation, target_geom_id, floor_geom_id=None):
...
def maybe_jitter_object_pose(mujoco, model, data, rng, enabled):
