...
class Deformation(nn.Module):
    ...
    def create_net(self):
        ...
        # predict a single isotropic Gaussian radius
        self.scales_deform = nn.Sequential(nn.ReLU(),nn.Linear(self.W,self.W),nn.ReLU(),nn.Linear(self.W, 1))
    ...
    def forward_dynamic(self,rays_pts_emb, scales_emb, rotations_emb, opacity_emb, shs_emb, time_feature, time_emb):
        ...
        # change slice, tensor shape one dementional
        if self.args.no_ds :
            scales = scales_emb[:,:1]
        else:
            ds = self.scales_deform(hidden)

            scales = torch.zeros_like(scales_emb[:,:1])
            scales = scales_emb[:,:1]*mask + ds
