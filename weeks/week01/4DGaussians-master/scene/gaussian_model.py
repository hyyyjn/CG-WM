...
class GaussianModel:
    ...
    def get_scaling(self):
        # store one Gaussian radius and expand it to xyz
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            # keep rotation fixed during optimization
    ...
    def construct_list_of_attributes(self):
        ...
        l.append('opacity')
        # keep exporting xyz-identical scales for compatibility with existing PLY readers
        for i in range(3):
    ...
    def save_ply(self, path):
        ...
        # export the stored radius to three identical raw scale channels
    ...
    def load_ply(self, path):
        ...
        # reduce loaded scale channels back to one isotropic radius
        scales = torch.tensor(scales, dtype=torch.float, device="cuda").mean(dim=1, keepdim=True)
        self._scaling = nn.Parameter(scales.requires_grad_(True))
        # keep loaded rotations frozen as well
    ...
    def _prune_optimizer(self, mask):
        ...
        for group in self.optimizer.param_groups:
            ...
            if stored_state is not None:
                ...
                del self.optimizer.state[group['params'][0]]
                requires_grad = group["name"] != "rotation"
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        # keep densified Gaussians isotropic with one stored radius
        new_scaling = self.scaling_inverse_activation(self.get_scaling[selected_pts_mask][:,:1].repeat(N,1) / (0.8*N))
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, density_threshold=20, displacement_scale=20, model_path=None, iteration=None, stage=None):
        ...
        # cloned points inherit the same isotropic radius and fixed rotation
    ...
    def standard_constaint(self):
        ...
        means3D_deform, scales_deform, rotations_deform, _, _ = self._deformation(means3D, scales, rotations, opacity, None, time)
